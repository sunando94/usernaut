...
func (suite *UserOffboardingJobTestSuite) SetupTest() {
	...
	suite.testUserKey = UserCacheKeyPrefix + suite.vinodUser.UserName
...
func (suite *UserOffboardingJobTestSuite) TearDownTest() {
	...
	_ = suite.cacheClient.Delete(suite.ctx, suite.testUserKey)
...
func (suite *UserOffboardingJobTestSuite) TestCompleteOffboardingFlow() {
	...
	// Step 2: Add Vinod to Redis cache
	suite.T().Log("Adding Vinod to Redis cache")
	vinodUserJSON, err := json.Marshal(suite.vinodUser)
	require.NoError(suite.T(), err, "Failed to marshal user data")

	err = suite.cacheClient.Set(suite.ctx, suite.testUserKey, string(vinodUserJSON), cache.NoExpiration)
	...
	cachedData, err := suite.cacheClient.Get(suite.ctx, suite.testUserKey)
	...
	if ldapErr != nil {
		...
		_, err = suite.cacheClient.Get(suite.ctx, suite.testUserKey)
